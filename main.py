...
def check_collision(grid, tetromino):
    for y, row in enumerate(tetromino.shape):
        for x, cell in enumerate(row):
            if cell and (tetromino.y + y >= len(grid) or tetromino.x + x >= len(grid[0]) or grid[tetromino.y + y][
                tetromino.x + x]):
                return True
    return False
...
def game():
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    tetromino.x -= 1
                    if check_collision(game_grid, tetromino):  # Используем game_grid вместо grid
                        tetromino.x += 1
        ...
        if pygame.time.get_ticks() - last_move > speed:
            ...
            if check_collision(game_grid, tetromino):
                tetromino.y -= 1
                merge_tetromino(game_grid, tetromino)  # Используем game_grid вместо grid
                remove_full_rows(game_grid)  # Используем game_grid вместо grid
                tetromino = Tetromino()
                if check_collision(game_grid, tetromino):
        ...
        for y, row in enumerate(game_grid):  # Используем game_grid вместо grid
            for x, cell in enumerate(row):
                if cell:
